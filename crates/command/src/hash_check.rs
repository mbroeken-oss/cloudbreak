...
fn unpack_snapshots(
    ...
    let mut all_files = sidecar::unpack_compressed_snapshot(full_path, full_slot)?.account_files;
    ...
    if let Some(inc) = incremental_snapshot {
        ...
        all_files
            .extend(sidecar::unpack_compressed_snapshot(inc_path, inc_slot)?.account_files);
    }
