...
| `cloudbreak_api_response_bytes_total`                | Counter           | `method`, `kind`       | Response bytes served. `kind` ∈ {`total` (all response bytes), `cached` (subset served from the GPA cache; `0` on paths that don't use the cache)}. Effective cache utilization is `kind="cached" / kind="total"` in Grafana. `method` matches the GPA/token-account `method` labels. |
| `cloudbreak_api_db_pool_connections`                 | Gauge             | `origin`               | SQLx database connection-pool state, sampled at scrape time. `origin` ∈ {`max` (configured `[database].max-connections`), `idle` (connections idle and immediately available), `total` (established connections, i.e. idle + in-use)}. In-use is `total - idle`; headroom before queries start waiting is `max - (total - idle)`, both computable in Grafana. |
| `cloudbreak_gpa_cache_size_bytes`                    | IntGauge          | —                      | Current size of the GPA cache in bytes. Pair with `cloudbreak_gpa_cache_max_bytes` to compute utilization (`size / max`). |
